...
int afe_dsm_ramp_dn_cfg(uint8_t *payload, int delay_in_ms)
{
	...
	*(params + 5)	= 5000;           // 5s mute time will make sure silence output till PA software shutdown.
...
int afe_dsm_get_calib(uint8_t* payload){
	return afe_dsm_setget_params(payload, sizeof(uint32_t)*15, 1, DSM_TX_PORT_ID, AFE_MODULE_DSM_TX, AFE_PARAM_ID_CALIB);
}
...
/*For Qulcomm android 8.0 reconsitution the audio folder*/
